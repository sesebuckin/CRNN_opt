...
def _get_input():
    ...
    """得到训练数据集的tensor,int,string等格式的数据，image格式为tensor"""
...
def main(argv=None):
    ...
    with tf.Graph().as_default():
        ...
        with tf.device(FLAGS.train_device):
            features,sequence_length = model.convnet_layers( image, width, mode) # 得到feature maps和特征向量序列长度
            logits = model.rnn_layers( features, sequence_length,
                                       mjsynth.num_classes() )      # 得到每一帧xt对应的标签yt
            train_op = _get_training(logits,label,sequence_length)  # 得到RNN网络的训练op节点
